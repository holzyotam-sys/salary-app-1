...
function calculateChildrenCreditPoints(form: Form101Data, today = new Date()): number {
  ...
  // אם נקודות הילדים שייכות לבן/בת הזוג בלבד, לא מזכים את המשתמש
  ...
  // אם חילקו נקודות בין ההורים
  if (form.childPointsReceiver === "split") {
    points = points / 2;
  }

  // ב-MVP הזה, הורה יחיד מקבל תוספת בסיסית
  if (form.singleParent && form.children.length > 0) {
    points += 1;
  }

  // גרוש/ה והילדים לא גרים אצלו/אצלה => לא נותנים את מלוא נקודות הילדים
  if (
